...
# Quarto separates [Content]{.contentchip .fragment .custom .turnsred data-fragment-index="3"} and [Format]{.formatchip}
...
# "An open-source<br>scientific and technical<br>publishing system"
:::
...
# [A modular *canvas*<br>for modular *content*]{.font120}
:::
...
# [Four ways an error gets stopped]{.center}

. . .

<br>

## [**The source runs**]{.center .darkgreen}
...
### [GitHub Actions runs it<br>on _every commit_]{.center}
...
<span class="sp-err">   ! all(funding$name %in% africa$name)</span>
</pre>
...
## [**The source is text**]{.center .darkgreen}

::: {.col .font140 width="60%" valign="middle"}
<!-- ONE artifact, two readings. The card never moves; what moves is the
     highlight. Click 1 makes the point the diff rows are already showing;
     click 2 steps those rows back to plain grey and lights the ev_sales line
     instead, so the eye is handed from one claim to the other on the same four
     lines of source. Row tint plus a darker token tint is the diff card's own
     idiom (.sp-add + .w), reused here in yellow because a computed value is
     CONTENT.

     NEVER write a live inline-R span anywhere on this slide -- not in the card,
     not in this comment. knitr runs before pandoc and evaluates backtick-r
     anywhere in the file, HTML comments included, so it would try to resolve
     ev_sales and fail with "object 'ev_sales' not found". The card shows the
     call by escaping both backticks as &#96;. -->
<pre class="specimen diffcard">
<span>  ## EV Sales</span>
<span> </span>
<span>  Sales of **electric vehicles** grew</span>
<span>  20% in 2025, reaching over</span>
<span class="steps-up fragment custom" data-fragment-index="2">  _<span class="w">&#96;r ev_sales&#96;</span> million_ sales globally.</span>
<span> </span>
<span class="sp-del steps-back fragment custom" data-fragment-index="2">- EVs now <span class="w">outsell</span> hybrids.</span>
<span class="sp-add steps-back fragment custom" data-fragment-index="2">+ EVs are <span class="w">closing the gap with</span> hybrids.</span>
</pre>
:::
...
### Plain text means<br>edits are **diffs**
:::

<br>

::: {.fragment data-fragment-index="2"}
### Inline R code means<br>**computed numbers**
:::
