...
Open by acknowledging the apparent mismatch: this is a dashboard session, but we are
starting with a handwriting worksheet. The worksheet is deliberately simple because it
makes the underlying problem obvious.

Say: "I promise we will get to dashboards. But this is the smallest example I could find
of a problem that gets much more interesting when the artifact gets complicated."
...
Let the request sound almost trivial. The point is not that the model cannot do it; the
point is that asking for a finished artifact entangles the content and the format.
...
This is the first generated artifact. Don't spend long critiquing it; the next slides make
the failure mode more useful.
...
Eight rounds later, the model has mostly converged, but only by repeatedly changing things
that were already right. This is the setup for separating content from format.
...
The Minecraft version is intentionally useful: the content changed, but what I really
wanted was for the format to stay fixed.
...
Say: "This is what I was actually asking for: these sentences, with this layout."
That is a compositional problem, not really a prompting problem.
...
Name the problem: the artifact is one entangled object. To change the content, the model
has to regenerate the thing that also controls the format.
...
Introduce the canvas idea. Quarto lets the format become a stable canvas while content is
supplied separately. This is the first major property: composition.
...
Show the payoff: one canvas, different content, same structure. Pokémon and Minecraft are
two inputs to the same compositional system.
...
Make the thesis explicit: "Ask for the .qmd, not the artifact." Then immediately scale the
idea up: "Now let's see what happens when the artifact is a dashboard."
...
Reveal the Vehicle Trends dashboard. This is the payoff for starting with the handwriting
example. The audience now sees that the same idea scales from a tiny PDF to a complex web
application.
...
Use this as the large-scale counterpart to the handwriting diagram. Say: "This is the same
idea at a much larger scale." The LLM is not being asked to understand one monolithic
dashboard; it writes a collection of Quarto pages, and Quarto composes the site.
...
Introduce modularity here. The dashboard is a Quarto website whose pages are independent
QMDs. Each page is effectively its own small canvas, while the website provides the
...
# [Just Code Chunks]{.font120}

::: {.notes}

This is the key architectural simplification: no dashboard framework is required. A page
can just be code chunks that render into a browser artifact.
...
Do not frame R versus Python as a competition. They are examples of different components
that can live inside the same Quarto canvas.

:::

---

## [Widgets that talk to each other: `crosstalk` + `plotly`]{.center}
...
The important point is composition: widgets can remain independent components and still
communicate. Quarto provides the page that contains them.
...
OJS is another implementation choice. The same visual idea can be recomputed in the
browser without changing the surrounding Quarto page.

:::

---

## [Use Shiny in a box!]{.center}
...
Shiny shows how far the canvas can stretch. The thing inside the page can be an entire
interactive application, whether it runs through Shinylive or a server.
...
The map pushes the point one step further: the component can even live in another
repository and be embedded. Quarto does not need to own the technology to compose it.
...
Briefly emphasize the deployment payoff, but don't turn this into a deployment talk:
the composed dashboard can still reduce operational complexity because much of the
artifact is just static files.
...
Now pivot from "what can the canvas contain?" to "why does this architecture matter for an
AI agent?" Introduce the source-versus-rendered-artifact comparison.
...
"All Markdown" is the conceptual bridge: underneath the rendered artifacts, we can work
with a compact, plain-text source representation.
...
Bring back the dashboard architecture mentally: fifteen small QMDs, not one giant object.
Highlight depreciation.qmd. Say: "If I ask the agent to change depreciation, it can reason
about this page rather than the whole dashboard."

This is where the dashboard diagram from earlier becomes the modularity argument.
...
This is the key AI line: "This isn't about how much you write. It's about how much has to
be true at once — and that is what an agent has to hold in its head."

Then make the causal chain explicit: modularity -> smaller context -> fewer tokens -> lower
cost. Cost is a consequence of modularity, not a separate pillar.
...
::: {.notes}
These are the three properties I want the audience to remember.

Composition: Quarto gives AI a canvas where different kinds of content can live together.

Modularity: that canvas is made of small pieces, so humans and agents don't have to hold the
whole system in their heads at once. Lower token cost follows from this.

Correctness: because the source is executable and inspectable, we can test it, render it,
and review exactly what changed.
:::
...
### `quarto render` errors _quickly_
...
Quarto makes the source executable. If an assertion fails, render stops. That turns a
silent artifact problem into an explicit failure that a machine can catch.
...
And source is also human-checkable. A plain-text QMD produces a meaningful diff. Humans can
see what changed rather than trying to compare two opaque rendered artifacts.

Together: machine checks execution; humans inspect the source.
...
Acknowledge the rough edges briefly. This is not a claim that AI + Quarto is frictionless.
The important comparison is that these failures are cheap to detect and fix when the
agent is working on source.

Do not dwell here; the next slide is the philosophical punchline.
...
# [Ask your agent for the `.qmd`.]{.font150}

::: {.notes}
Return to the handwriting example.

Say: "We started with a tiny artifact — a handwriting worksheet — and the lesson was not
that AI should make handwriting worksheets."

"The lesson was: don't ask the agent for the finished artifact. Ask it for the source."

Then: "Next time you're building something — a PDF, a report, a website, or a dashboard —
ask your agent to build you a .qmd and see where it takes you."
