...
viewof mass = {
  ...
  // Paint the filled-left portion blue like crosstalk's ionRangeSlider bar. A
  // native range has no cross-browser "progress" pseudo-element (WebKit has
  // none), so track the value as a CSS variable that .ojs-stack's track
  // gradient reads. Returning the same Inputs element keeps the viewof binding.
  const el = r.querySelector('input[type="range"]');
  const paint = () => {
